...
def query(dataset_id, dataset):
    ...
    if request.method == "POST":
        ...
        geostore = request.get_json().get("geostore", None)
        fs = copy.deepcopy(dict(**request.get_json()))
    else:
        sql = request.args.get("sql", None)
        geostore = request.args.get("geostore", None)
        fs = copy.deepcopy(dict(**request.args))
    result_query = build_query(dataset, sql=sql, fs=fs)
    try:
        if sql:
            query_type = "sql"
        else:
        ...
        logging.error(
            "[ROUTER] /query/<dataset_id> - SQL Format error in query conversion: "
            + e.message
        )
        return error(status=400, detail=e.message)
    except Exception as e:
        logging.error(
            "[ROUTER] /query/<dataset_id> - Generic error in query conversion: "
            + str(e)
        )
    ...
    try:
        ...
        logging.error(
            "[ROUTER] /query/<dataset_id> - GEE Query error in GEE query execution: "
            + e.message
        )
        return error(status=400, detail=e.message)
    except Exception as e:
        logging.error(
            "[ROUTER] /query/<dataset_id> - Generic error in GEE query execution: "
            + str(e)
        )
        return error(status=500, detail="Generic Error")
...
def fields(dataset_id, dataset):
    ...
    try:
        ...
        logging.error(
            "[ROUTER] /fields/<dataset_id> - GEE Query error in GEE query execution: "
            + e.message
        )
        return error(status=400, detail=e.message)
    except Exception as e:
        logging.error(
            "[ROUTER] /fields/<dataset_id> - Generic error in GEE query execution: "
            + str(e)
        )
        return error(status=500, detail="Generic Error")
...
def download(dataset_id, dataset):
    ...
    try:
        ...
        logging.error(
            "[ROUTER] /download/<dataset_id> - SQL Format error in query conversion: "
            + e.message
        )
        return error(status=400, detail=e.message)
    except Exception as e:
        logging.error(
            "[ROUTER] /download/<dataset_id> - Generic error in query conversion: "
            + str(e)
        )
        return error(status=500, detail="Generic Error")
    ...
    try:
        ...
        logging.error(
            "[ROUTER] /download/<dataset_id> - GEE Query error in GEE query execution: "
            + e.message
        )
        return error(status=400, detail=e.message)
    except Exception as e:
        logging.error(
            "[ROUTER] /download/<dataset_id> - Generic error in GEE query execution: "
            + str(e)
        )
        return error(status=500, detail="Generic Error")
...
def register_dataset():
    ...
    try:
        ...
        logging.error(
            "[ROUTER] /rest-datasets/gee - GEE Query error in GEE query execution: "
            + e.message
        )
        status = 2
    except Exception as e:
        logging.error(
            "[ROUTER] /rest-datasets/gee - Generic error in GEE query execution: "
            + str(e)
        )
        status = 2
