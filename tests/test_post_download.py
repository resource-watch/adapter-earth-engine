...
from tests.mocks import mock_convert_sql, mock_get_dataset
from tests.fixtures import (
    dataset_invalid_connector_type,
    dataset_invalid_provider,
    dataset_json,
    query_json as download_json,
)
...
def test_download_dataset_that_does_not_exist(client, mocker):
    ...
    response = client.post(
        "/api/v1/earthengine/download/bar", headers={"x-api-key": "api-key-test"}
    )
...
def test_download_dataset_no_download(client, mocker):
    ...
    response = client.post(
        "/api/v1/earthengine/download/bar",
...
def test_download_dataset_happy_case(client, mocker):
    mock_get_dataset(mocker, dataset_json)
    mock_request_validation(mocker, microservice_token=os.getenv("MICROSERVICE_TOKEN"))
    mock_convert_sql(mocker, download_json)
