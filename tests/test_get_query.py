...
def test_query_dataset_that_does_not_exist(client, mocker):
    json_response = {
        "errors": [{"status": 404, "detail": "Dataset with id 'foo' doesn't exist"}]
    }
...
def test_query_dataset_happy_case(client, mocker):
    ...
    mock_request_validation(mocker, microservice_token=os.getenv("MICROSERVICE_TOKEN"))
    response = client.get(
        "/api/v1/earthengine/query/bar?sql=SELECT%20ST_HISTOGRAM%28rast%2C%20elevation%2C%2010%2C%20true%29%20FROM%20CGIAR%2FSRTM90_V4",
        headers={"x-api-key": "api-key-test", "content-type": "application/json"},
    )
